...
function RenderGamingPCForm($PCList) {
    ...
    RefreshPCListBox -listBox $listBox
     ...
     $buttonUpdateImage = CreateButton "PC Image" 40 185
     ...
     $buttonRemove = CreateButton "Delete" 417 230
     $buttonRemove.Add_Click({
     ...
     $buttonUpdate = CreateButton "Update" 177 230
     ...
     $buttonAddNew = CreateButton "Add New" 297 230; $buttonAddNew.Add_Click({
             $checkboxNew.Checked = $true
             $buttonUpdate.PerformClick() | Out-Null
         }); 
     $gamingPCForm.Controls.Add($buttonAddNew)

     $buttonCancel = CreateButton "Cancel" 540 230; $buttonCancel.Add_Click({ 
             $listBox.Remove_SelectedIndexChanged({})
             $pictureBox.Image.Dispose(); $pictureBox.Dispose();
...
function RenderConfigureForm() {
     $configureForm =	CreateForm "Gaming Gaiden: Configure" 395 330 ".\icons\running.ico"
    ...
    if($null -ne $idleTimeConfig) {
     ...
     $autostartOptions = @("Enabled", "Disabled")
     $radioGroupAutostart = createRadioGroup -Options $autostartOptions -GroupTitle "Autostart:"
     $startupPath = "$env:APPDATA\Microsoft\Windows\Start Menu\Programs\StartUp"
     $shortcutExists = Test-Path "$startupPath\Gaming Gaiden.lnk"
     if ($shortcutExists) {
         $buttonToSelect = $radioGroupAutostart.Controls | Where-Object { $_.Text -eq "Enabled" }
         $buttonToSelect.Checked = $true
     } else {
         $buttonToSelect = $radioGroupAutostart.Controls | Where-Object { $_.Text -eq "Disabled" }
         $buttonToSelect.Checked = $true
     }
     $radioGroupAutostart.Anchor = "None"
     $table.Controls.Add($radioGroupAutostart, 0, 2)

     $configureForm.Controls.Add($table)

     $buttonOK = CreateButton "OK" 90 240
     $buttonOK.Add_Click({
        ...
        if ($null -ne $idleTimeChoice) {
         ...
         $autostartChoice = $radioGroupAutostart.Controls | Where-Object { $_.Checked -eq $true }
        if ($null -ne $autostartChoice) {
             $selectedValue = $autostartChoice.Text
            if ($selectedValue -eq "Enabled") {
                  $appDir = (Get-Location).Path
                  $appShortcutPath = Join-Path $appDir "Gaming Gaiden.lnk"
                 if (-Not (Test-Path $appShortcutPath)) {
                     $WshShell = New-Object -ComObject WScript.Shell
                     $Shortcut = $WshShell.CreateShortcut($appShortcutPath)
                     $Shortcut.TargetPath = Join-Path $appDir "GamingGaiden.exe"
                     $Shortcut.WorkingDirectory = $appDir
                     $Shortcut.Save()
                     Log "Created Gaming Gaiden shortcut in application directory."
                 }
                 Copy-Item -Path $appShortcutPath -Destination $startupPath -Force
                 Log "Autostart enabled. Shortcut copied to startup path."
             } elseif ($selectedValue -eq "Disabled") {
                 $startupShortcutPath = Join-Path $startupPath "Gaming Gaiden.lnk"
                 if (Test-Path $startupShortcutPath) {
                     Remove-Item $startupShortcutPath -Force
                     Log "Autostart disabled. Shortcut removed from startup path."
                 } else {
                     Log "Autostart disabled. Shortcut not found in startup path."
                 }
             }
         } else {
            Log "No Change in Autostart configuration"
         }
     ...
     $buttonCancel = CreateButton "Cancel" 215 240; $buttonCancel.Add_Click({ $configureForm.Dispose() }); $configureForm.Controls.Add($buttonCancel)
